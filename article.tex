...
sky-cameras, looking upward and capturing the sky lightfield.  This
...
Our reconstruction algorithm is based on the
Single-Scattering model (\cref{sec:single-scatt-model}). The
Single-Scattering approximation enables stating an analytic solution
to the Radiative Transfer in a closed form. This closed form is
beneficial to us as it enables us to invert the model and thus recover
the underlying atmosphere (\cref{sec:inverse-problem}). We validate
our reconstruction method by applying it on 'Realistic' images.
We simulate image acquisition using Monte Carlo (MC) simulations
(\cref{sec:monte-carlo-simul}).  The monte carlo method models
multiple scattering and therefore enables simulating realistic
scenarios. 
