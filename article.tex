...
\hl{We use finer vertical resolution to provide for higher variability
of air and aerosols distribution in the vertical dimension.} Each
...
\Cref{fig:cross_sections} plots the intensity profile
...
\Cref{fig:cross_sections}(a) compares photographs \cref{fig:simulation-results1}(d)
...
\Cref{fig:cross_sections}(b) compares photographs of the {\em Haze Blobs} scene
...
\begin{figure}[t]
  ...
  \caption{\small Cross-sections along the $X$-axis of photographs rendered
    ...
    (a) cross sections of the low density scene (\cref{fig:simulation-results1}(d)
    and \cref{fig:simulation_particles}(a)). (b) cross sections of
    the high density scene (\cref{fig:simulation_particles}(d)).
  }
...
multiple-scattering problems, mainly in 1D scenes (e.g. successive
orders of scattering and discrete ordinates method). We may tap into some of these approaches.
