...
\begin{align}
  ...
  [1+\rleq{(\cos \Phi^{\rm scatter})}^2]}
  {(2+g^2)(1 + g^2 - 2g\rleq{\cos \Phi^{\rm scatter}})^{\sfrac{3}{2}}}
...
F^{-1}(u)$, \rl{where $F^{-1}$ denotes the inverse function of $F$}.
...
the sky is discretized into a grid of $N_{\rm voxels}$ \rl{rectangular cuboid} volume elements
...
apply). \fix{In this model}{Here}, RT has three steps (\cref{fig:groundgrid}):
...
depth along the light path to and from voxel $k$ \del{as} (analyzed in
\cref{sec:optical-depth}).  Step~{\tt ii} involves the scattering
coefficient at voxel $k$ \del{described in} (\cref{sec:scattering}).
...
(\cref{eq:beer-lambert,eq:tautotal,eq:alphabasic}) the \fix{the radiant power contributed by is}{voxel contributes} radiant power
\begin{equation}
  p_c(k)= L^{\rm TOA}
...
\rl{In all cases, the vertical resolution was higher than horizontal, via use
...
\cref{eq:bigIA}. \endnote{\rl{For clarity of display, the brightness of the
displayed pictures in this paper underwent the same standard
contrast enhancement (stretching), including gamma-correction.
The recovery algorithms, of course, have used the raw (not brightness
enhanced) images.}}
...
\begin{align}
  ...
  \label{eq:MCextinct}
\end{align}
Distance $l^{\rm random}$ along ${\cal R}_{i}$ yields \del{the} 3D position ${\bf
...
randomly \fix{according to their relative densities at the voxel containing
${\bf X}_{i}$.}{The relative probabilities of this random step are set by the respective extinction coefficients
($\beta^{\rm air}$ vs. $\beta^{\rm aerosol}$) at the voxel containing ${\bf X}_{i}$.}


\noindent ({\tt iv}) If \fix{it}{the particle} is an aerosol, the photon-packet intensity
...
estimation~\cite{marshak20053d} \fix{is used for calculating}{derives} the amount of
...
from the same viewpoint as \cref{fig:simulation-results1}(a). \fix{A
comparison of a sample horizontal cross section is shown in
Fig.~\ref{fig:mcgrid}c.}
{Figure~\ref{fig:cross_sections} plots the intensity profile
...
Figure~\ref{fig:cross_sections}(a) compares photographs Fig.~\ref{fig:simulation-results1}(d)
and Fig.~\ref{fig:simulation_particles}(a) of the {\em Haze Blobs} scene.
This plot demonstrates consistency in this scene, for most pixels.
Figure~\ref{fig:cross_sections}(b) compares photographs of the {\em Haze Blobs} scene
where the aerosol density was increased ten folds (see Fig.~\ref{sec:optimization-results}
and the corresponding MC photograph Fig.~\ref{fig:simulation_particles}(d)).
...
\begin{figure}[t]
  ...
  \caption{\small \rl{Cross-sections along the $X$-axis of photographs rendered
    by the single-scattering (dotted) and MC (solid line) forward models.
    (a) cross sections of the {\em Haze Blobs} scene (Fig.~\ref{fig:simulation-results1}(d)
    and Fig.~\ref{fig:simulation_particles}(a)). The difference between the models is much more pronounced (b) 
    when the scatterers are ten times denser, corresponding to Fig.~\ref{fig:simulation_particles}(d).}
  }
  \label{fig:cross_sections}
\end{figure}

\rl{Figure~\ref{fig:scatter_order} shows the contribution by successive
...
Fig.~\ref{fig:scatter_order}(a). Similarly, contributing photons that had
experienced exactly  two or three scattering events yield respectively
Fig.~\ref{fig:scatter_order}(b) and Fig.~\ref{fig:scatter_order}(c). First-order
...
\begin{figure}[bt]
  ...
  \caption{\small \rl{Separation of the image shown in Fig.~\ref{fig:mcgrid}(b) to contributions
...
\end{equation}
Here $\MaskSun$ masks the area around the sun: \del{as} in real-world images
it is indeed blocked. \rl{We mask the horizon, by using a camera whose
...
scattering (Fig.~\ref{fig:scatter_order}(b)). Consequently,
...
and reliable. \del{As seen in Fig.~\ref{fig:groundgrid},}Voxels at high
...
\rl{To demonstrate recovery, we performed simulations, and tested effects of the following variations:
...
\rl{In all cases, images were created as described
in Secs.~\ref{sec:simul} and~\ref{sec:monte-carlo-simul}. These images served as input to the reconstruction algorithm. The optimization used an L-BFGS-B solver}~\cite{BFGS} \rl{on a computer cluster. Each computer core was dedicated to rendering a modeled image. The optimization was initialized
...
\rl{\noindent{\bf Forward models and distributions}:
Figure~\ref{fig:results} shows estimation results, corresponding to the
original distributions of Fig.~\ref{fig:distributions}.}
\begin{figure}[t]
  \centering
  \yoavcomment{\includegraphics{images/results}}
  \caption{\small \fix{Reconstructions of the}{Recoveries of} scenes shown in
    \cref{fig:distributions}. Color represents aerosol density\fix{. The density units are}{, in
    units of} $10^{6}~{\rm particles}/{\rm m}^3.$ In \fix{[a,b],
    single-scattering simulated images}{(a) and (b)
    images simulated by single-scattering} are used as input \fix{to}{for} the
    recovery.  In (c) and (d), MC simulated images are used as input for the
    recovery.  }
  \label{fig:results}
\end{figure}
\rl{Here, the atmosphere domain was defined over a
...
\begin{table}[t]
  ...
  \begin{tabular}{cccccgg}
    ...
    \bottomrule
  \end{tabular}
  \caption{\rl{Relative errors in various simulations. Here ${\mathcal{O}}$ denotes order of magnitude.}}
  \label{tbl:mass}
\end{table}
\rl{the algorithm of} \cref{sec:inverse-problem}. \\


\rl{\noindent{\bf Density of viewpoints}: Keeping the domain and scene ({\em Haze blobs}) fixed, we
...
Figure~\ref{fig:error_vs_cameras} plots the performance.}
\begin{figure}[t]
  \centering
  \yoavcomment{\includegraphics{images/error_vs_cameras.pdf}}
  \caption{\small \rl{The relative error $\epsilon$ measure decreases with the number of
    cameras. Bars represent the standard deviation of $\epsilon$, over our random tests.}}
  \label{fig:error_vs_cameras}
\end{figure}
\rl{Diminishing returns are obtained beyond $N_{\rm views}\approx 20$, which corresponds to $\approx 11$km
...
\rl{\noindent{\bf Aerosol characteristics}: We used the {\em Haze blobs} scene, but varied the
aerosol characteristics. Compared to the aerosol described in Sec.~\ref{sec:simul}, we tested two variations. In one variation, the phase function was isotropic, i.e.,
...
\begin{figure}[b]
  ...
  \caption{\small \rl{Images simulated by MC, from the same viewpoint, but different
...
\end{figure}
\rl{Recovery results using $N_{\rm views}=36$ are shown in Figs.~\ref{fig:reconstructions}(a)
and \ref{fig:reconstructions}(b)}
\begin{figure}[b]
  \centering
  \yoavcomment{\includegraphics{images/reconstructions.pdf}}
  \caption{\small  \rl{Recovered distributions when the aerosol is either party absorbing (a),
    has isotropic phase function (b) or has high density (c).}}
  \label{fig:reconstructions}
\end{figure}
\rl{and in Table~\ref{tbl:mass}. The results indicate that if the aerosol's phase function
...
\rl{\noindent{\bf Density scale}: We used the aerosol {\em Haze blobs} distribution
...
of high order scattering, as shown in Fig.~\ref{fig:cross_sections}(b) and
Fig.~\ref{fig:simulation_particles}(d). Consequently, $\epsilon$ increases (Table~\ref{tbl:mass}),
when using $N_{\rm views}=36$. Still, Fig.~\ref{fig:reconstructions}(c) shows
...
sensing depart from common \fix{1D}{one-dimensional} aerosol distribution models, in favor
...
optical depth is small. However, as \del{the} MC simulations show, it is
important to generalize \st{is algorithm so that it explicitly
...
\st{To deploy such a system on a very large scale will require a long-term
engineering effort. Camera specifications}~\cite{Pust2011}\st{should be
set to optimally recover a wide range of aerosol distributions. This
requires a major theoretic and numerical research.}
\rl{the recovery algorithm so that high-order scattering
...
\rl{should be set to optimally recover a wide range of aerosol distributions.}
...
\fix{We thank David~J. Diner for useful discussions. Yoav Schechner is a
...
NASA.}{We are grateful to David~J. Diner for fruitful discussions and his support at all
