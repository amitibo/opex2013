...
Fig.~\ref{fig:mcgrid}b shows a photograph of the {\em Haze Blobs}
scene, derived by the MC process, using \num{10000} photons per pixel,
from the same viewpoint as Fig.~\ref{fig:simulation-results1}[left].  A
...
Sec.~\ref{sec:simul} describes simulated scenes at various grid
...
% Using the recovered distribution shown in Figs.~\ref{fig:results}c
% (originating from MC), we ran the single scattering model to create
% images. \cref{fig:simulation-results1}c shows a cross of such an
% image compared to the original MC image.
