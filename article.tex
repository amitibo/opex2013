...
The atmosphere is discretized into a grid of volume elements (voxels).
A forward model takes as input the aerosol distribution over the grid
of voxels, and outputs images as if captured at various viewpoints.
In a \emph{single-scattering} model, any SR changes direction only
once on the way to the camera. This model is valid in atmospheres that
are not very dense: inside fog and clouds this model does not apply.
