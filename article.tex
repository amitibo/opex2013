...
\begin{table}[htbp]
  ...
  \begin{tabular}{cccccg}
    \toprule
    Scene  & Forward & Aerosol & Isotropic     & Absorbing & $\epsilon$ \\
           & model&   density & Phase function? & aerosol?   & \\
     \midrule
    Haze Front & Single scatter & Low & & & 0.18 \\
          & Monte Carlo & Low & & & 0.65 \\
    \cmidrule(r){1-5}
    Haze Blobs & Single scatter & Low & & & 0.14 \\
     & Monte Carlo & Low & & & 0.65 \\
     & Monte Carlo & Low & $\surd$  & & 0.49 \\
     & Monte Carlo & Low & & $\surd$ & 0.83 \\
     & Monte Carlo & High & & & 0.98 \\
    \bottomrule
  \end{tabular}
  \caption{Relative error measure in various simulations}
  \label{tbl:mass}
\end{table}
...
\begin{figure}[h]
  ...
  aerosol characteristics. (a) Haze blobs, characterized in Sec.~\ref{sec:simul}.
    high. (b) Partly absorbing aerosol. (c) Aerosol having an isotropic phase function. (d) High aerosol density.}
...
\begin{figure}[h]
  ...
  \caption{\small  Recovered distributions when the aerosol is either party absorbing (a),
    isotropic (b) or highly dense (c).}
  \label{fig:reconstructions}
\end{figure}
and in Table~\ref{tbl:mass}.\\
...
sensing depart from common 1D aerosol distribution models, in favor
...
in our tests, the MC forward model was slower by 2-3 orders of magnitude than a single-scattering analytic model. Thus, inverting a 3D multiple-scattering model requires research into
numerical schemes for increasing the solution efficiency of both forward and inverse problems.  
The remote sensing community has devised several approaches to better approximate multiple-scattering problems, mainly in 1D scenes. These include successive orders of scattering, DISORT..... We may tap into some of these approaches.
Deploying an experimental system will be needed: camera specifications~\cite{Pust2011} should be
