...
cross sections of the viewed field, extracted from the images in \cref{fig:simulation-results1}[left] and \cref{fig:mcgrid}(b).
...
\begin{figure}[t]
  ...
  \caption{\small Cross sections along the $X$-axis [a] and $Y$-axis [b] of
    and images rendered by either single-scattering and MC. The corresponding
    images appear in \cref{fig:simulation-results1}[left] and \cref{fig:mcgrid}(b).}
  \label{fig:cross_sections}
\end{figure}

\Cref{fig:scatter_order} shows the contribution by successive
scattering orders. They are derived from the MC image. Photons contributing to an image pixel are accumulated in steps {\tt ii},{\tt v} above.
...
\begin{figure}[bt]
  ...
  \caption{\small Separation of the image shown in \cref{fig:mcgrid}(b) to contribution by successive orders of scattering: (a) first  order, (b) second order and (c) third scattering order.}
...
Sample results are shown in Fig.~\ref{fig:variations}(a,b,c,d) and in Table~\ref{tbl:mass}.\\

\noindent{\bf Density scale}: We used the aerosol {\em Haze blobs} distribution and characteristics as described in Sec.~\ref{sec:simul}, but increased the aerosol density tenfold everywhere. This significantly increases the effects of high order scattering, as shown in Fig.~\ref{fig:cross_sections}(b) and Fig.~\ref{fig:variations}(e). Consequently, $\epsilon$ increases, when using $N_{\rm views}=36$. Still, Fig.~\ref{fig:variations}(f) shows that the spatial distribution outlines are recovered similarly to the other tests.
...
% \begin{figure}[h]
%   \centering
%   \yoavcomment{\includegraphics{images/high_density_results.pdf}}
