...
randomly by the relative \hl{scattering coefficients
($\vect{\alpha}^{\rm air}$ and $\vect{\alpha}^{\rm aerosol}$} respectively)
at the voxel containing ${\bf X}_{i}$.
...
Figure~\ref{fig:cross_sections} plots the intensity profile in two lateral
cross sections of the viewed field, extracted from the images in
\cref{fig:simulation-results1}(d) and the corresponding MC derived photograph.
These plots demonstrate consistency in these cases, for most pixels.
\begin{figure}[t]
  \centering
  ...
  \caption{\small Cross-sections along the $X$-axis of photographs rendered
    by the single-scattering and MC forward models. 
    (a) cross sections of low density simulations (\cref{fig:simulation-results1}(d)
    and \cref{fig:simulation_particles}(a)). (b) cross sections of
    high density simulations (\cref{fig:simulation_particles}(d)).
  }
  \label{fig:cross_sections}
\end{figure}

\Cref{fig:scatter_order} shows the contribution by successive
scattering orders. They are derived from the MC image. Photons contributing
to an image pixel are accumulated in steps {\tt ii},{\tt v} above.
...
\cref{fig:scatter_order}(c). Clearly, first-order scatter is responsible
for most of the energy in the MC image. Radiance by higher order scattering
...
\begin{figure}[bt]
  ...
  \caption{\small Separation of the image shown in \cref{fig:mcgrid}(b) to contribution by successive orders of scattering: (a) first , (b) second, (c) third and (d) forth scattering order.}
...
it is indeed blocked. We mask the horizon, by using a camera whose field of view is a bit smaller than hemispherical. This reduces the influence of image regions that are more strongly affected by high-order scattering (\cref{fig:scatter_order}(b)). Consequently,
the fit of the image formation model focuses on the bulk image region, where the single scattering model is more valid.  In \cref{eq:objectiveA}, $\Psi({\bm n})$
...
To demonstrate recovery, we performed simulations. We tested how results are affected by the following variations:
...
in \cref{sec:simul,sec:monte-carlo-simul}. These images served as input to the reconstruction algorithm. The optimization used an L-BFGS-B solver~\cite{BFGS} on a computer cluster. Each computer core was dedicated to rendering a modeled image. The algorithm was initialized
...
\begin{figure}[h]
  ...
  \caption{\small Reconstructions of the scenes shown in
    ...
    density units are $10^{6}~{\rm particles}/{\rm m}^3.$ In (a) and (b)
...
The overall mass is recovered, but for 6.5\% loss. \\
...
repeated the reconstruction many times, each time using a different subset of the above mentioned 36 simulated cameras. Each test randomly selected viewpoints, for various fixed values
of $N_{\rm views}< 36$. All rendered images used MC.
\Cref{fig:error_vs_cameras} plots $\epsilon$ as a function $N_{\rm views}$. 
\begin{figure}[t]
  \centering
  \yoavcomment{\includegraphics{images/error_vs_cameras.pdf}}
  \caption{\small Relative error $\epsilon$ measure as function of the number of
    cameras. Bars represent the standard deviation of $\epsilon$, over our random tests.}
  \label{fig:error_vs_cameras}
\end{figure}
Diminishing returns are obtained beyond $N_{\rm views}\approx 20$, which corresponds to $\approx 11$km separation between neighboring viewpoints.\\
...
[0.69,0.74,0.78]$.  All rendered images used MC, and recovery used $N_{\rm views}=36$.
Sample results are shown in Figs.~\ref{fig:simulation_particles}(a)--(d) and in Table~\ref{tbl:mass}.\\
...
\begin{table}[htbp]
  ...
  \begin{tabular}{cccccg}
    \toprule
    Haze Scene & Forward Model& Density & Isotropic PF & Absorbing & $\epsilon$ \\
    \midrule
    Front & Single & Low & & & 0.18 \\
     & MC & Low & & & 0.65 \\
    \cmidrule(r){1-5}
    Blobs & Single & Low & & & 0.14 \\
     & MC & Low & & & 0.65 \\
     & MC & Low & $\surd$  & & 0.49 \\
     & MC & Low & & $\surd$ & 0.83 \\
     & MC & High & & & 0.98 \\
    \bottomrule
  \end{tabular}
  \caption{Local error for the various reconstruction simulations}
  \label{tbl:mass}
\end{table}
...
\cref{fig:simulation_particles}(c). Consequently, $\epsilon$ increases,
...
\begin{figure}[h]
  ...
  \caption{\small MC simulations of the {\em Haze Blobs} scene
    with varying density scales and aerosol characteristics
    (described in \cref{sec:optimization-results}). All images
    correspond to the same viewpoint of
    \ref{fig:simulation-results1}(d). (a) low density scale
    (b) absorbing particle, (c) isotropic particle and (d) high
    density scale.
  }
...
\begin{figure}[h]
  ...
  \caption{\small 
    Reconstructions of the scenes shown in
    Figs.~\ref{fig:simulation_particles}(b)--(d).
    (a) absorbing particle scene, (b) isotropic particle scene
    and (c) high density scale scene, respectively.}
...
clouds. \Cref{fig:scatter_order} \hl{suggests that incorporating
