...
apply). \rl{Here}, RT has three steps (\cref{fig:groundgrid}):
...
(\cref{eq:beer-lambert,eq:tautotal,eq:alphabasic}) the \rl{voxel contributes} radiant power
\begin{equation}
  p_c(k)= L^{\rm TOA}
...
randomly \rl{The relative probabilities of this random step are set by the respective extinction coefficients
($\beta^{\rm air}$ vs. $\beta^{\rm aerosol}$) at the voxel containing ${\bf X}_{i}$.}


\noindent ({\tt iv}) If \rl{the particle} is an aerosol, the photon-packet intensity
...
estimation~\cite{marshak20053d} \rl{derives} the amount of
...
\rl{Figure~\ref{fig:cross_sections} plots the intensity profile
...
\begin{figure}[t]
  ...
  \caption{\small \rl{Recoveries of} scenes shown in
    \cref{fig:distributions}. Color represents aerosol density\rl{, in
    units of} $10^{6}~{\rm particles}/{\rm m}^3.$ In \rl{(a) and (b)
    images simulated by single-scattering} are used as input \rl{for} the
...
sensing depart from common \rl{one-dimensional} aerosol distribution models, in favor
...
\rl{We are grateful to David~J. Diner for fruitful discussions and his support at all
