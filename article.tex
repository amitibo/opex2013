...
\begin{abstract}
  ...
  radiative transfer (RT). We propose a principle to estimate the aerosol  
  distribution as it really is: a three dimensional (3D) volume, using visible light. 
  The principle is a type of tomography. The process involves
  wide angle integral imaging of the sky on a very large scale.  
  We formulate an image formation model based
  on 3D RT. Model inversion is done using optimization
  methods, exploiting a closed-form gradient which we derive for the
  model-fit cost function.  The tomography model is distinct, as the radiation
...
Optical lightfield and integral imaging~\cite{bishop,horstmeyer,kim,Ng1948}
...
with a far larger scale, to estimate scatterer 3D spatial distribution in the sky. 
The atmosphere allows light to pass through in multiple locations and directions. Light is affected
by this medium. Therefore, in this paper we lay out a principle to
recover this 3D distribution using measured and modeled light-fields
...
In remote sensing~\cite{diner}, imaging through air is associated with {\em
  atmospheric correction}, based on {\em aerosol retrieval}. In this
discipline, the atmosphere is often assumed to be plane-parallel, using one-dimensional vertical RT. Consequently state-of-the-art aerosol retrieval is done in distinct large lateral
blocks~\cite{Martonchikc} with limited height
resolution~\cite{kalashnikova}. We, however, seek 3D
recovery. Atmospheric correction is related to dehazing~\cite{Joshi2010,Zhu2013}. In
this paper, however, the medium itself, at all relevant altitudes is the object of interest.


We rely on sky imaging from multiple directions and locations. Such projection is similar to
tomography in other scientific domains~\cite{Arridge}. However, the situation here
...
tomography can detect gases~\cite{Kazahaya2008,Price2001,Cosofret2009,wright}, which absorb
UV or IR radiation. However, aerosol attenuation of visible
light is typically dominated by {\em scattering}, rather than absorption. Since the radiation source is single and fixed in space and time, we cannot rely on direct illumination for tomographic recovery of attenuation fields~\cite{Aviles2011}. We may only use sunlight scattered into the LOS. The model is
{\em nonlinear, yet tractable}. We model passive optical tomographic imaging of 3D atmospheric
...
\noindent {\bf Inverse Transform Sampling}: Monte Carlo methods
...
camera $c$.
%Similarly, the single-scattering albdeos $\varpi^{\rm aerosol}$ in all voxels are stacked to vector $\vect{\varpi}$.
...
%Most generally, this is the set
%$\left\{{\bm n},\left\{\vect{\varpi}_{\lambda},\vect{\sigma}_{\lambda},{\bm
%        g}_{\lambda}\right\}_{\lambda}\right\}$
%where some of the parameters depend on the spectral channel $\lambda$.
%Usually, a single aerosol type dominates a scene. Furthermore, we assume that the type of the aerosol above an area is known~\cite{Martonchik2009}. Hence, we focus on estimating the density distribution ${\bm n}$.
...
in close-form,
\begin{align}
%  {\bm J}_{{\bm i}_c}({\bm n}) &= L^{\rm TOA}(\abbrev{A}-\abbrev{B})\abbrev{C}
...
%    ]}
%    %\nonumber \\
%  %\abbrev{C} &= \OpDiag{\exp[-(\vect{\tau}_c^{\rm air} + {\sigma}^{\rm
%   %   aerosol} {\bm D}_c {\bm n})]} \transpose{{\vect{\Pi}}_c}
%\end{align}
\begin{equation}
  \label{eq:gradient3}
...
\noindent{\bf Cameras}: The acquired images are cresimulated using the the MC model.
