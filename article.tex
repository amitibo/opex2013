...
imaging~\cite{bishop,horstmeyer,kim,Ng1948} samples the optical
...
In remote sensing~\cite{diner}, imaging through air is associated with
...
dehazing~\cite{Joshi2010,Zhu2013}. In this paper, however, the medium
...
domains~\cite{Arridge}. However, the situation here is distinct. Most
tomography setups have a controlled and/or multidirectional radiation
source~\cite{Boas2001,messer}. In contrast, our source is the
...
gases~\cite{Kazahaya2008,Price2001,Cosofret2009,wright}, which absorb
...
(\cref{sec:theor-backgr}).  The CDF of the transmittance distribution
is given by,
\begin{equation}
  \label{eq:1}
  F=\int_{0}^{1}\exp(-\tau)\derivsym{\tau}=1-\exp(-\tau)
\end{equation}
Given a uniformly distributed random number $u$ a random transmittance
is given by
\begin{equation}
  \label{eq:2}
  t=F^{-1}(u)=-\ln(1-u)
\end{equation}
If the scattering occurs outside the atmospheric grid, the photon is
dropped. If not, the voxel in which the scattering occurs is
...
\end{figure}
To test the model, we simulated several atmospheric models. Here are
the details.
...
different grid sizes from \num{50 x 50 x 100} grid, where each voxel
has area of \SI{1 x 1}{\km} and vertical thickness of \SI{100}{\metre}
and down to \num{10 x 10 x 20} grid, where each voxel has area of
\SI{5 x 5}{\km} and vertical thickness of \SI{500}{\metre}.
...
photograph of the {\em Haze Blobs} scene that corresponds to
...
% i}_c}({\bm n})$ from \cref{eq:gradient1}. First show some results
% relating to differentiation.  Let
% $\vect{a}(\vect{n}),\vect{u}(\vect{n})$ be vector functions: each
% outputs a vector of length $r$. Let $\mat{C}$ be a $r \times N_{\rm
% voxels}$ matrix, where $N_{\rm voxels}$ is the length of ${\bm n}$.
% Then,
% \begin{align}
%   \label{eq:partial1}
%   \PartDeriv{\transpose{\mat{C}} (\vect{a} \odot
%   \vect{u})}{\vect{n}}
%   = \left[ \PartDeriv{\vect{a}}{\vect{n}} \OpDiag{\vect{u}} +
%     \PartDeriv{\vect{u}}{\vect{n}} \OpDiag{\vect{a}} \right]
%   \mat{C}.
% \end{align}
% Here we denote by $\OpDiag{\vect{v}}$ conversion of a general vector
% $\vect{v}$ into a diagonal matrix, whose main diagonal elements
% correspond to the elements of $\vect{v}$. Now, let
%% \begin{align}
%   $\vect{a}(\vect{n}) = \exp(-{\mat{C}}\vect{n})$,
