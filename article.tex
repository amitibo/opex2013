...
are encapsulated in the vector ${\bm{\Theta}}$. Any camera $c$ is at a
distinct fixed location ${\bm x}$, capturing raw image
$i_c({\bm{\Theta}})$. This paper takes the first step, of formulating
a principle of recovering the 3D atmospheric aerosols distribution,
...
derives an analytic closed form solution to the radiative
transfer. This closed form enables simple inversion of the model, to
...
\sigma^{\rm aerosol}(k),\varpi^{\rm aerosol}(k), n^{\rm
  aerosol}(k),g(k)\}$ are constants, e.g., corresponding to the value
...
\begin{align}
  ...
  \exp\{-(\vect{\tau}_c^{\rm air} + {\sigma}^{\rm aerosol} {\bm D}_c
  {\bm n})\}\odot [\tilde{\vect{\alpha}}^{\rm air}_c + \varpi^{\rm
    aerosol}\sigma^{\rm aerosol} P^{\rm aerosol}_g(\vect{\Phi}^{\rm
    scatter}_c) \odot{\bm n} ] \Big\} .
  \label{eq:bigIA}
\end{align}
...
coefficient.  We use $\Mask$ as a weight map to mask the influence of
the sun and the horizon. These regions are prone to dominant multiple
scattering.  One common measurement of smoothness are the second order
derivatives.  These can be calculated using the \emph{Laplacian}
operator, $\nabla^2$.  We enforce smoothness of the solution by
minimizing the signal energy of the \emph{Laplacian} of the signal
\begin{equation}
  \label{eq:regularizer}
  \Psi({\bm n}) = \| \nabla^2{\bm n}\|^2_2 = \| \Laplacian{\bm n}\|^2_2
\end{equation}
where $\Laplacian$ is a matrix representation of the \emph{Laplacian}
operator.
...
$\partial i_c({\bm \Theta})/\partial{n^{\rm aerosol}(k)}$.  A
close-form expression of our Jacobian and its detailed derivation is
...
differentiation. Let $\vect{n}$ be a vector of length $m$. Let
$\vect{a}(\vect{n})$ be a vector function: it outputs a vector of
length $r$. Let $\mat{C}$ be a $m \times r$ matrix and
\begin{align}
  \vect{a}(\vect{n}) = \exp[-\transpose{\mat{C}}\vect{n}].
  \label{eq:example1}
\end{align}
...
\begin{align}
  \label{eq:partial2}
  \PartDeriv{\vect{a}}{\vect{n}} &= - \mat{C} \,
  \OpDiag{\exp[-\transpose{\mat{C}}\vect{n}]}
\end{align}
...
function that outputs a vector of length $r$. Let $\mat{C}$ be a $r
\times m$ matrix. Then,
\begin{align}
  \label{eq:partial1}
