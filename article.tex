...
N.~J. Pust, A.~R. Dahlberg, M.~J. Thomas, and J.~a. Shaw, \enquote{{Comparison
  ...
  simulations which employ AERONET products.}} Optics express \textbf{19},
  18602--13 (2011).
...
I.~Ihrke, K.~N. Kutulakos, H.~P. Lensch, M.~Magnor, and W.~Heidrich,
  ...
  reconstruction},} in \enquote{EUROGRAPHICS 2008 STAR--STATE OF THE ART
  REPORT,}  (2008).
...
In remote sensing~\cite{Pust2011, Mishchenko2007}, imaging through air is
...
\begin{align}
  ...
  \label{eq:alphabasic}
\end{align}
% Let $\mu\equiv \cos\Phi^{\rm scatter}$.
...
intersection line segment is $l_{\rm LOS_c}(m|\Delta z,\Phi^{\rm
  LOS}_c)$.  As in \cref{eq:Dsv}, define a $N_{\rm voxels}\times
...
\begin{align}
  ...
  \label{eq:lSRn}
\end{align}
Let $\vect{\tau}_{\rm SR}$ and $\vect{\beta}^{\rm air}$ be column
stack vector representations of $\tau_{\rm SR}(k)$ and $\beta^{\rm
  air}(k)$, respectively.  Then, we can write \cref{eq:lSRn} using
matrix notation: \mbox{$\vect{\tau}_{\rm SR}= \OpDistance^{\mathrm{Sun
      \rightarrow voxel}}\vect{\beta}^{\rm air} + \sigma^{\rm
    aerosol}\OpDistance^{\mathrm{Sun \rightarrow voxel}}
  {\DistUnknown}$},\ and \mbox{$\vect{\tau}_{{\rm LOS}_c}=
  \OpDistance^{\mathrm{voxel \rightarrow cam}}\vect{\beta}^{\rm air} +
  \sigma^{\rm aerosol}\OpDistance^{\mathrm{voxel \rightarrow cam}}
  {\DistUnknown}$}.  The SR and LOS paths joining at each voxel form a
...
%
%
% and $\vect{\tau}_{\rm SR}^{\rm air}$ be column stack vector
% representations of $\tau_{\rm SR}(k)$ and $\tau_{\rm SR}^{\rm
% air}(k)$, respectively.
%
% A similar analysis yields the optical depth along a LOS to voxel
% $k$, $\tau_{{\rm LOS}_c}(k)$. It is expressed by element $k$ of
...
where ${\bm i}_c$ is the image, column-stacked to a vector $N_{\rm
  pix}$ long.  Combining
\cref{eq:tautotal,eq:pc,eq:IcP}, the captured image is thus
\begin{align}
...
\begin{figure}
  ...
  \caption{\small [a] Multiple-scattering forward model by Monte
    Carlo. [b] Photograph of the same sky and view point as in
    \cref{fig:simulation-results1}, rendered using a MC model.  [c]
    Cross section along the $X$-Axis of the MC simulation,
...
original distributions of \cref{fig:distributions}. In
\cref{fig:results}a,b, the single-scattering model was used both
in rendering of the raw images, and in the estimation algorithm. In
\cref{fig:results}c,d, the MC model (multiple scattering)
...
Errors in \cref{fig:results}a,b stem from random noise, which had
...
error can be quantifies by the aerosol mass that is over and under-estimated
...
\hat{\bm n} - {\bm n}^{\rm true} \|_1 / \| {\bm n}^{\rm true} \|_1$.
  In
\cref{fig:results}a,b, respectively $\epsilon=0.14$ and
$\epsilon=0.20$.  In \cref{fig:results}c,d, both cases have
$\epsilon=0.65$.
...
engineering effort. Camera specifications should be set to optimally
recover a wide range of aerosol distributions. This requires a major
theoretic and numerical research.
